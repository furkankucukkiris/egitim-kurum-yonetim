...
export function StatusRatioBar({ segments }: { segments: StatusRatioDatum[] }) {
  ...
  return (
    <div>
      {total <= 0 ? (
        <p className="py-6 text-center text-sm text-muted">Bu aralıkta tahakkuk kaydı yok.</p>
      ) : (
        <>
          <div
            className="flex h-4 w-full overflow-hidden rounded-full bg-fill"
            role="img"
            aria-label={segments
              .map((s) => `${s.label}: ${formatTry(s.amount)} (%${Math.round((s.amount / total) * 100)})`)
              .join(", ")}
          >
            {nonZero.map((segment) => (
              <div
