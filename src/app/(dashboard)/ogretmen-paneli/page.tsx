            ...
            {groups.map((group) => (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <span className="rounded-full bg-honey-50 dark:bg-honey-500/10 px-3 py-1 text-xs font-bold text-honey-700 dark:text-honey-500">
                    {
                      weekdayLabels[
                        group.weekday
                      ]
              ...
              return (
                ...
                >
                  <td className="px-5 py-4">
                    ...
                    {enrollment.status ===
                      "frozen" && (
                      <span className="mt-2 inline-block rounded-full bg-honey-100 dark:bg-honey-500/15 px-2 py-1 text-xs font-semibold text-honey-700 dark:text-honey-500">
                        Donduruldu
                      </span>
                    )}
                  </td>
...
function MebBadge({
  ...
  if (status === "registered") {
    return (
      <span className="rounded-full bg-emerald-100 dark:bg-emerald-500/15 px-2.5 py-1 text-xs font-bold text-emerald-800 dark:text-emerald-400">
        MEB kayıtlı
      </span>
    );
  }
  ...
  if (status === "pending") {
    return (
      <span className="rounded-full bg-honey-100 dark:bg-honey-500/15 px-2.5 py-1 text-xs font-bold text-honey-700 dark:text-honey-500">
        MEB kaydı bekliyor
      </span>
    );
  }
  ...
  return (
    <span className="rounded-full bg-fill px-2.5 py-1 text-xs font-semibold text-muted">
      MEB kayıtlı değil
    </span>
  );
}
...
function EmptyState({
  ...
  return (
    <div className="rounded-2xl border border-line bg-panel p-8 text-center text-sm text-muted shadow-sm">
      {children}
    </div>
  );
}
