...
export async function generateMonthlySessions(formData: FormData) {
  await requireRole(["admin"]);

  const month = String(formData.get("month") ?? "").trim();

  if (!isMonthValue(month)) {
    redirect(`/yoklama?error=${encodeURIComponent("Geçerli bir ay seçmelisiniz.")}`);
  }

  const supabase = await createClient();

  const { data, error } = await supabase.rpc("generate_monthly_lesson_sessions", {
    p_month_start: `${month}-01`,
  });

  if (error) {
    console.error("Aylık ders oturumları oluşturulamadı:", error);
  ...
  const result = ((data ?? []) as GenerationResult[])[0] ?? {
    created_count: 0,
    existing_count: 0,
  ...
  const messageParts = [`${result.created_count} yeni ders oturumu oluşturuldu.`];

  if (result.existing_count > 0) {
    messageParts.push(`${result.existing_count} oturum daha önce oluşturulmuştu.`);
  }
  ...
  const targetDate = result.first_session_date ?? `${month}-01`;

  redirect(`/yoklama?date=${targetDate}&success=${encodeURIComponent(messageParts.join(" "))}`);
}
